...
export function TenantAdminResourceEditPage() {
  ...
  const save = async () => {
    if (!tenant?.id || !resourceId || !resource) return;
    setSaving(true);
    try {
      const isLink = resource.type === 'link';
      const payload: Record<string, unknown> = { title, description, type: resource.type };
      if (isLink) {
      ...
      <div className="space-y-6 relative">
        ...
        {loading ? (
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-gray-200 p-6 space-y-6">
            ...
            {/* Link resource: read-only note (no URL editing) */}
            {resource.type === 'link' && (
              <div className="rounded-lg border border-amber-200 bg-amber-50/80 p-4 text-sm text-amber-800">
                This is an external link resource. You can only edit the title and description above. The link URL cannot be changed.
              </div>
            )}
